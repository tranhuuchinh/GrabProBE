...
export const setupMediator = async (queues: string[]) => {
  ...
  async function createQueue(queueName: string) {
    const queueInfo = await new Promise<amqp.Replies.AssertQueue>((resolve, reject) => {
      if (channel) {
        channel.assertQueue(queueName, { exclusive: true }, (err, q) => {
          if (err) {
            reject(err)
          } else {
            resolve(q)
...
export const consumeFromMediator = (queue: string) => {
      ...
      channel.consume(
        ...
        (msg: any) => {
          if (msg && msg.content) {
            ...
            console.log(queueMapping[queue])
            resolve(queueMapping[queue])
          }
        },
        { noAck: false }
      )
